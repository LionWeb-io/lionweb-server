...
import { FeatureMissing, PropertyAdded, PropertyDeleted, PropertyValueChanged } from "@lionweb/json-diff"
...
const AddPropertyFunction = async (
    ...
    const result = await ctx.dbConnection.tx(async (task: LionWebTask) => {
        ...
        deltaLogger.debug(`PropertyAdded: db add dor ${msg.newValue} result is ${JSON.stringify(dbResult)}`)
...
const DeletePropertyFunction = async (
    ...
    const result = await ctx.dbConnection.tx(async (task: LionWebTask) => {
        ...
        // OKI, now store the new value
        const change = new PropertyDeleted(
            new JsonContext(null, ["delta"]),
            msg.node,
            msg.property,
            oldProperty.value,
