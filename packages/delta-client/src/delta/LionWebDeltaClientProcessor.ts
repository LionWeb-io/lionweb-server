...
import { DeltaEvent, DeltaResponse } from "@lionweb/server-delta-shared"
import { ValidationResult } from "@lionweb/validation"
import { IEventProcessor } from "./events/IEventProcessor.js"
import { IResponseProcessor } from "./queryresponses/index.js"

type MessageFromServer = DeltaEvent | DeltaResponse
...
export class LionWebDeltaClientProcessor {
    ...
    constructor(queries: IResponseProcessor, events: IEventProcessor) {
    ...
    processDelta(socket: WebSocket, delta: DeltaEvent): void {
        ...
        if (func === undefined) {
            console.error(`2 processDelta: processing function not found for '${type}'`)
            return
        }
    ...
    initialize(queries: IResponseProcessor, events: IEventProcessor) {
        ...
        this.processingFunctions.set(
            ...
            events.ChildMovedFromOtherContainmentInSameParentFunction as ProcessingFunction
        )
        ...
        this.processingFunctions.set(
            ...
            events.ChildMovedAndReplacedFromOtherContainmentFunction as ProcessingFunction
        )
        this.processingFunctions.set(
            "ChildMovedAndReplacedFromOtherContainmentInSameParent",
            events.ChildMovedAndReplacedFromOtherContainmentInSameParentFunction as ProcessingFunction
        )
        ...
        this.processingFunctions.set(
            ...
            events.AnnotationMovedAndReplacedFromOtherParentFunction as ProcessingFunction
        )
        ...
        this.processingFunctions.set(
            ...
            events.EntryMovedAndReplacedFromOtherReferenceInSameParentFunction as ProcessingFunction
        )
