...
export async function getRepositoryData(task: LionWebTask, request: Request, defaultClient?: string): Promise<RepositoryData | ParameterError> {
    ...
    if (isParameterError(clientId)) {
        ...
        const repo: RepositoryInfo = await repositoryStore.getRepository(task, repoName)
...
export class RepositoryStore {
    ...
    async getRepository(task: LionWebTask, repoName: string): Promise<RepositoryInfo> {
        if (!this.initialized) {
            await this.initialize(task)
        }
