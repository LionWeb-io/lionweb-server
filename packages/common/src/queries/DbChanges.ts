...
export class DbChanges {
    ...
    addChanges(changes: Change[]): void {
        changes.forEach(change => {
            switch (change.changeType) {
                ...
                case "NodeClassifierChanged": {
                    const update: DbNodeUpdate = {
                        column: "classifier",
                        id: (change as NodeClassifierChanged).node.id,
                        newValue: (change as NodeClassifierChanged).newClassifier
                    }
                    this.updatesOnNodeTable.add(update.id, update)
                    break
                }
                default: throw new Error(`DbChanges.addChanges: case ${change.changeType} missing`, )
            }
        })
    }
    ...
    createPostgresQuery(metaPointersTracker: MetaPointersTracker): string {
        ...
        this.updatesOnNodeTable.values().forEach((values: DbNodeUpdate[]) => {
            ...
            values.forEach(v => {
                if (v.column === "classifier") {
                    newValue[v.column] = metaPointersTracker.forMetaPointer(v.newValue as LionWebJsonMetaPointer)
                } else {
                    newValue[v.column] = v.newValue
                }
            })
            result += `-- update nodes
                        UPDATE ${NODES_TABLE}
    ...
    async populateMetaPointersFromDbChanges(
        ...
        const classifierChangesToDb: DbNodeUpdate[] = []
        for(const changesPerNode of this.updatesOnNodeTable.values()) {
            const classifierChanges = changesPerNode.filter(change => change.column === "classifier") 
            if (classifierChanges.length > 0) {
                classifierChangesToDb.push(classifierChanges[classifierChanges.length - 1])
            }
        }
        await metaPointersTracker.populate(collector => {
            classifierChangesToDb.forEach(change=> collector.considerAddingMetaPointer(change.newValue as LionWebJsonMetaPointer))
